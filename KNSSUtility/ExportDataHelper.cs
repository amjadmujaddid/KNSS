...
namespace KNSSUtility
{
    public class ExportDataHelper
    {
        ...
        //public static Dictionary<string, string> ExportToExcel(string fileName, string worksheetName, string header, string sql, List<string> isCenter = null)
        //{
        ...
        //    var allData = Db.GetMultiDataColName(Db.Query(sql));
        ...
        //    var data = SqlForAdminHelper.GetFieldAndType(sql);
        ...
        //            sheet.Cell(columnName).Value = row[fldName];
        //            iSheetValue++;

        //            if (DateTime.TryParse(row[fldName], out tmpDate))
        //            {
        ...
        //            }
        //            else if (decimal.TryParse(row[fldName], out tmpDecimal))
        //            {
        //                sheet.Cell(columnName).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Right;
        //            }
        //            else if (int.TryParse(row[fldName], out tmpInt))
        //            {
        //                sheet.Cell(columnName).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Right;
        //            }
        ...
        public static Dictionary<string, string> ExportToExcel<T>(string fileName, string worksheetName, List<T> dataFromService, List<string> isCenter = null) where T : class
        {
            ...
            foreach (Dictionary<string, string> x in col)
            {
                ...
                {
                    columnName = Library.GetColumnExcel(i) + iSheetValue;
                    sheet.Cell(columnName).Value = row;
                    iSheetValue++;

                    if (DateTime.TryParse(row.ToString(), out tmpDate))
                    {
                        sheet.Cell(columnName).Style.DateFormat.Format = "dd-MMM-yyyy";
                        sheet.Cell(columnName).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
                    }
                    else if (decimal.TryParse(row.ToString(), out tmpDecimal))
                    {
                        sheet.Cell(columnName).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Right;
                    }
                    else if (int.TryParse(row.ToString(), out tmpInt))
                    {
                        sheet.Cell(columnName).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Right;
                    }
